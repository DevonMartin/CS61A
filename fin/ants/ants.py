    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                if place.ant.contained_ant is None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        if random_or_none(self.place.bees) != None and min_range <= i <= max_range:
            return random_or_none(self.place.bees)
        next = self.place.entrance
        while next != None and next != beehive:
            i += 1
            if random_or_none(next.bees) != None and min_range <= i <= max_range:
                return random_or_none(next.bees)
            next = next.entrance
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        return super().nearest_bee(beehive)
    # END Problem 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif self.digesting == 0 and bee is not None:
            self.eat_bee(bee)
        # END Problem 6
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing but hault bees with it's 4 armor."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is the water-safe version of a ThrowerAnt, costing slightly more."""
    ...
    def action(self, gamestate):
        ...
        if self.is_imposter:
            ...
            while place is not None:
                ant = place.ant
                if ant:
                    if not ant.got_doubled:
                        ant.damage *= 2
                        ant.got_doubled = True
                    if isinstance(ant, ContainerAnt):
                        cAnt = ant.contained_ant
                        if cAnt:
                            if not cAnt.got_doubled:
                                cAnt.damage *= 2
                                cAnt.got_doubled = True
                place = place.exit
